...
- `READY`: 담당자 착수 검토, AI 재검증, Ready Check, 위험도 확정, 필요한 ADR, Human 구현 승인을 모두 통과한 작업 계약이다.
...
9. Issue 범위, 예상 변경 파일, 테스트, 제외 범위를 짧게 계획하고 보고한다.
10. Issue 범위 안에서 최소 변경으로 코드와 테스트를 구현한다.
11. 가장 작은 관련 테스트부터 전체 테스트·빌드·diff 검증까지 실행한다.
12. 실제 diff를 다시 읽어 범위 이탈, 핵심 버그, 테스트 누락을 자체 리뷰한다.
13. Issue 범위 안의 수정 가능한 문제는 고치고 재검증한다.
14. 실행한 명령과 결과, 테스트 보장·비보장 범위, 직접 검증, 미검증 항목을 보고한다.
15. 검증이 끝나면 작업 브랜치에 Commit·Push하고 Draft PR을 생성한다.
16. PR 번호를 Human에게 전달하고 위험도에 맞는 리뷰를 기다린다.
17. 승인된 리뷰 수정은 같은 브랜치와 같은 PR에 반영한다.
