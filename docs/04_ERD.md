...
`outbox_events.aggregate_id`에는 주문 ID가 저장되지만 현재 JPA Entity에서 `Order` 연관관계 FK로 매핑하지 않는다. 이벤트 재처리에 필요한 aggregate 식별값으로 관리한다. 현재 애플리케이션은 주문 성공 시 `ORDER_COMPLETED` 이벤트 1개를 만들지만 DB는 주문당 이벤트 수를 UNIQUE 제약으로 강제하지 않는다.
...
users 1 : N orders
menu  1 : N orders
orders 1 : N outbox_events  (논리 관계, JPA FK·주문당 개수 제약 미매핑)
```

- 한 사용자는 하나의 포인트 지갑을 가진다.
- 한 사용자는 여러 포인트 이력과 주문을 가질 수 있다.
- 하나의 메뉴는 여러 주문에서 참조될 수 있다.
- 현재 애플리케이션 흐름은 주문 성공 시 하나의 `ORDER_COMPLETED` Outbox 이벤트를 저장한다.
- DB는 `aggregate_id`를 FK 또는 UNIQUE로 강제하지 않으므로 향후 주문당 여러 이벤트 타입을 확장할 수 있다.
...
- 현재 서비스는 주문당 1개 이벤트를 만들지만 `aggregate_id + event_type` UNIQUE 제약이 없어 DB가 중복 생성을 방지하지 않는다.
