...
- v0에서는 메뉴 조회, 포인트 충전, 주문·결제를 포함한 필수 API 구현에 필요한 Entity와 Repository를 생성한다. `Order`와 주문 Repository는 Issue #8 범위에 포함된다. Docker Compose, Redis, RabbitMQ, MySQL Replication 설정은 Issue #8 범위가 아니다.
- v1에서는 #10에서 동시 주문 문제를 재현하고, #11에서 DB 비관적 락 적용과 포인트 정합성을 검증한다.
